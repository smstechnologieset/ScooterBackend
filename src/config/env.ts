...
const envSchema = z.object({
  ...
  COMMAND_TIMEOUT_MS: z.coerce.number().int().positive().default(15000),
  COMMAND_RETRY_ATTEMPTS: z.coerce.number().int().nonnegative().default(2),
  ...
  PROTOCOL_INBOUND_LENGTH_MODE: z
    .enum(["unspecified", "total-bytes", "body-bytes", "command-and-payload-bytes"])
    .default("unspecified"),
  PROTOCOL_OUTBOUND_LENGTH_MODE: z
    .enum(["unspecified", "total-bytes", "body-bytes", "command-and-payload-bytes", "literal"])
    .default("unspecified"),
  PROTOCOL_OUTBOUND_LENGTH_LITERAL: z.string().optional(),
  PROTOCOL_CHECKSUM_MODE: z.enum(["unspecified", "none", "trailing-field"]).default("unspecified"),
  PROTOCOL_INBOUND_ACK_MODE: z.enum(["ack-prefix-command", "none"]).default("ack-prefix-command"),
  PROTOCOL_OUTBOUND_ACK_MODE: z.enum(["unspecified", "ack-prefix-command", "none"]).default("unspecified"),
  PROTOCOL_DUPLICATE_PACKET_MODE: z.enum(["record-only", "ack-again", "drop"]).default("record-only"),
  PROTOCOL_AUTH_FLOW: z.enum(["unspecified", "author-command", "disabled"]).default("unspecified")
});
